# Secure Certificate Generation demo

Deploy AAP with various RBACs to simulate a secure environment for certificate generation
...
-e ocp_username=kubeadmin \
-e ocp_password="<OpenShift Admin Password>" \
-e aap_manifest_path="<Local Path to AAP Manifest ZIP File for Licensing AAP>" \
-e demo_name=demo_secure_certgen \
./playbooks/_deploy_demo_on_ocp.yml`

Or to run on an existing AAP instance:

`ansible-playbook -i <inventory_file> \
-e controller_hostname=<AAP Hostname> \
-e controller_password="<AAP admin Password>" \
-e eda_host=<EDA Hostname> \
-e eda_password="<EDA Admin Password>" \
-e demo_name=demo_secure_certgen \
./playbooks/_deploy_demo.yml`
